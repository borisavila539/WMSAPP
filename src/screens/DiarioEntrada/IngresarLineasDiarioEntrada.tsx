...
type props = StackScreenProps<RootStackParams, "IngresarLineasDiarioEntrada">

export const IngresarLineasDiarioEntrada: FC<props> = ({ navigation }) => {
    ...
    const AgregarEliminarArticulo = async (PROCESO: string) => {
        ...
        try {
            await WmSApi.get<string>(`EntradaMovimiento/${WMSState.diario}/${barCode}/${PROCESO}`).then(x => {
                if (x.data == 'OK') {
                    getData();
                } else {
                    setbarcode('')
