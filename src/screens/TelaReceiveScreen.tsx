...
export const TelaReceiveScreen : FC<props> = ({ navigation })=> {
    ...
    const VerificarRollo = async () => {
      if (data.find(x => x.inventserialid == InventSerialID)?.inventserialid != '') {
        try {
          await WmSApi.get<DespachoPickingpackingInterface[]>(`DespachoTelaPickingPacking/${InventSerialID}/RECEIVE/-/-/${data.find(x => x.inventserialid == InventSerialID)?.transferid}`).then(x => {
            if (x.data.length > 0) {
              if (x.data[0].receive) {
                setinventSerialID('')
                getData()
    ...
    const renderItem = (item: DespachoTelaDetalleInterface) => {
      ...
      return (
        <View style={{ width: '100%', alignItems: 'center' }}>
          <View style={{ width: '90%', backgroundColor: !item.receive ? orange : blue, borderRadius: 10, marginBottom: 5, padding: 5 }}>
